 ...
 *
 * The client can request an encrypted payload (with JNLP MAC code as the key) or if the client has a suitable
 * permission, it can request a plain text payload.
 *
...
public class EncryptedSlaveAgentJnlpFile implements HttpResponse {
    ...
    /**
     * The object that owns the Jelly view that renders JNLP file. This is typically a {@link SlaveComputer} and if so
     * we'll use {@link SlaveComputer#getJnlpMac()} to determine the secret HMAC code.
     */
    ...
    /**
     * Name of the agent, which is used to determine secret HMAC code if {@link #it} is not a {@link SlaveComputer}.
     */
    ...
    public void generateResponse(StaplerRequest req, final StaplerResponse res, Object node) throws IOException, ServletException {
        ...
        if ("true".equals(req.getParameter("encrypt"))) {
            try (final CapturingServletOutputStream csos = new CapturingServletOutputStream()) {
